...
try {
    ...
    $fontDir = __DIR__ . '/assets/fonts/th-sarabun-psk'; $tempDir = __DIR__ . '/tmp/mpdf'; if (!is_dir($tempDir)) mkdir($tempDir,0777,true);
    ...
    $filename='องค์ประกอบที่3_'.preg_replace('/[^\p{L}\p{N}_-]+/u','_',$assessment['fullname']).'.pdf'; header('Content-Type: application/pdf'); header("Content-Disposition: inline; filename=\"component3.pdf\"; filename*=UTF-8''".rawurlencode($filename)); header('Content-Length: '.strlen($pdf)); echo $pdf; exit;
