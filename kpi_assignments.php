...
$eligiblePrimaryUsers = kpiEligiblePrimaryUsers($pdo, $userId, $role);
$eligibleIds = array_map(fn($u)=>(int)$u['id'], $eligibleUsers);
$eligiblePrimaryIds = array_map(fn($u)=>(int)$u['id'], $eligiblePrimaryUsers);
$managedIds = array_values(array_unique([...$eligibleIds, ...$eligiblePrimaryIds]));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        foreach ($indicatorIds as $indicatorId) {
            ...
            if (!in_array($primaryId,$eligiblePrimaryIds,true)) throw new RuntimeException('ผู้รับผิดชอบหลักอยู่นอกหน่วยงานที่รับผิดชอบ');
        ...
        $delete = null;
        if ($managedIds) {
            $marks = implode(',', array_fill(0, count($managedIds), '?'));
            $delete = $pdo->prepare("DELETE FROM kpi_assignments WHERE indicator_id=? AND user_id IN ($marks)");
        }
        $insert = $pdo->prepare('INSERT INTO kpi_assignments (indicator_id,user_id,responsibility_type,assigned_by) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE responsibility_type=VALUES(responsibility_type),assigned_by=VALUES(assigned_by)');
        foreach ($rowsToSave as [$indicatorId,$primaryId,$secondaryIds]) {
            if ($delete) $delete->execute([$indicatorId, ...$managedIds]);
...
if ($managedIds) {
    $marks=implode(',',array_fill(0,count($managedIds),'?'));
    $sql="SELECT a.indicator_id,a.responsibility_type,a.user_id,a.assigned_by FROM kpi_assignments a JOIN kpi_indicators k ON k.id=a.indicator_id WHERE k.cycle_id=? AND a.user_id IN ($marks) ORDER BY a.id";
    $stmt=$pdo->prepare($sql); $stmt->execute([$cycleId,...$managedIds]);
    foreach ($stmt->fetchAll() as $assignment) {
        $indicatorId = (int)$assignment['indicator_id'];
        if ($assignment['responsibility_type'] === 'primary') {
            $isExplicitAssignment = (int)$assignment['assigned_by'] !== (int)$assignment['user_id'];
            if (!isset($assignments[$indicatorId]['primary']) || $isExplicitAssignment) {
...
?>
<div class="card" style="margin-bottom:1rem"><div class="card-header"><div><h2 class="card-title"><?= appIcon('link') ?> กำหนดผู้รับผิดชอบตัวชี้วัด</h2><p style="color:var(--text-muted);margin:.4rem 0 0"><?= $role==='director'?'ผอ.รพ.สต. เป็นผู้รับผิดชอบหลักทุกตัวชี้วัด และกำหนดผู้รับผิดชอบรองภายใน รพ.สต. ของตน':'สสอ. และแอดมิน สสอ. กำหนดผู้รับผิดชอบตัวชี้วัดของ สสอ. โดยผู้ช่วย สสอ. รับผิดชอบทุกตัวชี้วัดอัตโนมัติ' ?></p></div></div></div>
...
<?php foreach($indicators as $k): $indicatorId=(int)$k['id']; $primaryId=$assignments[$indicatorId]['primary']??0; $secondaryIds=$assignments[$indicatorId]['secondary']??[]; ?>
  <tr>
    ...
    <td><select class="form-control" name="primary_user_id[<?= $indicatorId ?>]"><option value="0">— เลือกผู้รับผิดชอบหลัก —</option><?php foreach($eligiblePrimaryUsers as $u): ?><option value="<?= (int)$u['id'] ?>" <?= $primaryId===(int)$u['id']?'selected':'' ?>><?= htmlspecialchars($u['fullname']) ?> — <?= htmlspecialchars(kpiAssigneeRoleLabel((string)$u['role'])) ?> — <?= htmlspecialchars($u['short_name']) ?></option><?php endforeach; ?></select></td>
    ...
    <td><button class="btn btn-secondary" type="submit" name="save_indicator_id" value="<?= $indicatorId ?>" <?= !$eligiblePrimaryUsers?'disabled':'' ?>><?= appIcon('save') ?> บันทึกแถวนี้</button></td>
  </tr>
<?php endforeach; ?>
