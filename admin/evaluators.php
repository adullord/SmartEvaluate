...
$cycleId = (int)($_REQUEST['cycle_id'] ?? 0);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $action = $_POST['action'] ?? '';
        if ($action === 'assign') {
            $evaluateeId=(int)$_POST['evaluatee_id']; $evaluatorId=(int)$_POST['evaluator_id'];
        ...
        if ($action === 'remove') {
            $pdo->prepare('DELETE FROM evaluator_mapping WHERE id=?')->execute([(int)$_POST['id']]);
