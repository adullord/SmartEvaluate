...
<?php foreach ($assessments as $index => $row): ?><tr><td class="center"><?= $index + 1 ?></td><td><strong><?= htmlspecialchars($row['fullname']) ?></strong></td><td><?= htmlspecialchars($row['position_name'] . ' ' . $row['rank_name']) ?></td><td><?= htmlspecialchars($row['department_name'] ?: $row['service_code']) ?></td><td><span class="badge <?= $row['status'] === 'submitted' ? 'badge-success' : 'badge-primary' ?>"><?= htmlspecialchars(component3StatusLabel($row['status'])) ?></span></td><td class="center"><strong><?= number_format((float)$row['final_score'], 2) ?></strong></td><td class="center"><a class="btn btn-sm btn-primary" href="<?= htmlspecialchars(appUrl('export_component3_pdf.php')) ?>?id=<?= (int)$row['id'] ?>"><?= appIcon('download') ?> PDF</a></td></tr><?php endforeach; ?>
